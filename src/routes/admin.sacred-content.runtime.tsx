...
function SacredRuntimePage() {
  ...
  return (
    <div>
      ...
      <h2 className="mt-8 text-sm font-medium tracking-widest text-emerald-500 uppercase">
        Eligible now ({eligible.length})
      </h2>
      <RuntimeTable rows={eligible} empty="No runtime-eligible content yet." />

      <h2 className="mt-8 text-sm font-medium tracking-widest text-amber-500 uppercase">
...
function RuntimeTable({
  ...
  if (rows.length === 0) {
    return <p className="mt-3 text-sm text-stone-500">{empty}</p>
  }
  return (
      ...
      <table className="w-full min-w-[800px] border-separate border-spacing-0 text-sm">
        <thead>
          <tr className="text-left text-xs tracking-wider text-stone-500 uppercase">
            ...
            <th className="border-b border-stone-800 px-3 py-2">Lang</th>
            <th className="border-b border-stone-800 px-3 py-2">Version</th>
            <th className="border-b border-stone-800 px-3 py-2">Rights</th>
            <th className="border-b border-stone-800 px-3 py-2">Enabled</th>
            {showFailures ? (
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.versionId} className="hover:bg-stone-900">
